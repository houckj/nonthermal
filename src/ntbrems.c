...
/*{{{ Case 2. Non-Stationary Target */
static double wrap_ee_sigma (Brems_Type *b, double gm1) /*{{{*/
{
   double s_ee;
...
static double wrap_ei_sigma (Brems_Type *b, double gm1) /*{{{*/
{
   return _ntb_ei_sigma (gm1, b->photon_energy);
}

/*}}}*/

static Particle_Type *Target;
static double (*Wrap_Sigma)(Brems_Type *, double);

static double ns_mu_integrand (double mu, void *pt) /*{{{*/
{
   Brems_Type *b = (Brems_Type *)pt;
   Particle_Type *elec = b->electrons;
   Particle_Type *targ = Target;
   double gb_r, gb_t, br2, bt2;
   double gm1, s, radical;

   gb_r = b->pc_r / elec->mass / C_SQUARED;
   gb_t = b->pc_t / targ->mass / C_SQUARED;
   br2 = gb_r * gb_r / (1.0 + gb_r * gb_r);
   bt2 = gb_t * gb_t / (1.0 + gb_t * gb_t);

#define BETA2_TOL 1.e-6
   /* Setting this tolerance too small leads to convergence
    * problems, especially for low photon energies << m_e c^2
    */

   /* Transition from stationary target regime
    * to non-stationary target regime
    */
   if (bt2 < BETA2_TOL * br2)
     {
        double pcomc2, gamma, beta;
        pcomc2 = b->pc_r / elec->mass / C_SQUARED;
        gamma = sqrt (1.0 + pcomc2 * pcomc2);
        beta = sqrt ((1.0 + 1.0/gamma)*(1.0 - 1.0/gamma));

        radical = beta * GSL_CONST_CGSM_SPEED_OF_LIGHT;
        gm1 = gamma - 1.0;
     }
   else
     {
        double bc, beta_diff2, beta_cross2, beta_rel2;
        bc = sqrt (br2 * bt2) * mu;
        beta_diff2 = br2 + bt2 - 2 * bc;
        beta_cross2 = br2 * bt2 * (1.0 + mu) * (1.0 - mu);
        beta_rel2 = (beta_diff2 - beta_cross2) / (1.0 - bc) / (1.0 - bc);

        if (isnan(beta_rel2)
            || (beta_rel2 < 0))/*  || 1 < beta_rel2)) */
          {
             if (isnan(beta_rel2))
               fprintf (stderr, "beta_rel2=NaN\n");
             else
               {
                  fprintf (stderr, "beta_rel2=%20.17e\n", beta_rel2);
                  fprintf (stderr, "beta_rel2-1=%20.17e\n", beta_rel2-1);
                  fprintf (stderr, "bt2=%15.9e  br2=%15.9e  mu=%12.4e\n", bt2, br2, mu);
                  fprintf (stderr, "beta_diff2=%15.9e   beta_cross2=%15.9e  bc=%15.9e\n",
                           beta_diff2, beta_cross2, bc);
               }
             exit(1);
          }

        radical = sqrt (beta_diff2 - beta_cross2) * GSL_CONST_CGSM_SPEED_OF_LIGHT;

        if (1.0 - beta_rel2 > 0.0)
          {
             double gm_rel = 1.0/sqrt(1.0 - beta_rel2);
             gm1 = gm_rel - 1.0;
             if (gm1 > GAMMA_MAX_DEFAULT) gm1 = GAMMA_MAX_DEFAULT;
          }
        else gm1 = GAMMA_MAX_DEFAULT;
     }
...
static int integral_over_ns_mu (Brems_Type *b, double *val) /*{{{*/
{
   gsl_error_handler_t *gsl_error_handler;
   gsl_function f;
   double epsabs, epsrel, abserr;
   ...
   status = gsl_integration_qng (&f, mu_min, mu_max, epsabs, epsrel,
                                 val, &abserr, &neval);
   ...
   if (status)
     {
        /* FIXME */
#if 0
        fprintf (stderr, "*** ntbrem (ns_mu_integral): %s\n",
                 gsl_strerror (status));
#endif
     }
...
static double ns_targ_integrand (double pc_targ, void *pt) /*{{{*/
{
   Brems_Type *b = (Brems_Type *)pt;
   Particle_Type *targ = Target;
   double nt, s;
   ...
   b->pc_t = pc_targ;
   (void) integral_over_ns_mu (b, &s);
   (void)(*targ->spectrum) (targ, pc_targ, &nt);
...
static int integral_over_ns_targ (Brems_Type *b, double *val) /*{{{*/
{
   gsl_error_handler_t *gsl_error_handler;
   gsl_function f;
   double epsabs, epsrel, abserr;
   ...
   pc_min = (*targ->momentum_min)(targ);
   pc_max = (*targ->momentum_max)(targ);

   f.function = &ns_targ_integrand;
   f.params = b;
   epsabs = 0.0;
   epsrel = 1.e-6;

   gsl_error_handler = gsl_set_error_handler_off ();
   status = gsl_integration_qng (&f, pc_min, pc_max, epsabs, epsrel,
                                 val, &abserr, &neval);
   gsl_set_error_handler (gsl_error_handler);
...
#if 0
        /* FIXME */
        fprintf (stderr, "*** ntbrem (ns_targ_integral): %s\n",
                 gsl_strerror (status));
#endif
     }
...
static double ntb_integrand_non_stationary (double pc_elec, void *pt) /*{{{*/
{
   Brems_Type *b = (Brems_Type *)pt;
   Particle_Type *elec = b->electrons;
   double ne, s;
   ...
   b->pc_r = pc_elec;
   (void) integral_over_ns_targ (b, &s);

   /* Factor 1/2 from angular integration over isotropic
    * particle distribution functions.
    * see e.g. Dermer 1984 ApJ 280, 328, equation 3.
    * or Weaver 1976 Phys Rev A 13, 1563, equation 6.
    */
   s *= 0.5;

   (void)(*elec->spectrum) (elec, pc_elec, &ne);
...
int ntb_brems_non_stationary (void *vb, double photon_energy, double *emissivity) /*{{{*/
{
   Brems_Type *b = (Brems_Type *)vb;
   double (*integrand)(double, void *) = &ntb_integrand_non_stationary;
   double epsrel = 1.e-4;
   ...
   *emissivity = 0.0;
   ...
   fprintf (stderr, "photon_energy=%15.9e", photon_energy);
   ...
   if (b->ee_weight > 0.0)
     {
        ...
        Target = b->e_target;
        Wrap_Sigma = &wrap_ee_sigma;

        if (-1 == integral_over_electrons (b, integrand, epsrel, &s_ee))
          return -1;

        fprintf (stderr, " s_ee=%15.9e", s_ee);

        *emissivity += b->ee_weight * s_ee;
     }
   ...
   if (b->ep_weight > 0.0)
     {
        ...
        Target = b->i_target;
        Wrap_Sigma = &wrap_ei_sigma;

        if (-1 == integral_over_electrons (b, integrand, epsrel, &s_ep))
          return -1;
        fprintf (stderr, " s_ep=%15.9e", s_ep);

        *emissivity += b->ep_weight * s_ep;
     }
